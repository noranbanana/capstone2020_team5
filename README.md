...
- **머신러닝은 무엇을 적용하였나요?**
    - 하루 활동 중 수면패턴을 분리하기 위해 비지도학습에서의 군집분석 머신러닝인 클러스터링 알고리즘을 적용하였습니다. (머신러닝은 지도학습, 비지도 학습으로 나뉘며 비지도학습은 label이 없는 데이터에서 패턴을 발견하고, 숨겨진 구조를 찾아내는 것이고 이중 클러스터링 알고리즘이 있습니다)

- **활동량은 어떻게 계산했나요?**
    - 반려동물의 질량에 3축 가속도의 제곱합을 곱하여 운동에너지를 구하고 칼로리로 변환하는 상수를 나누어서 구했습니다.
calorie = dog_mass * (accX*accX + accY*accY + accZ*accZ) / (200.0 * 4.187);
...
- **홈페이지는 어떻게 유지하고 있나요?**
    - 정적 파일을 제공하는 express.static 을 사용하여 html파일이 있는 서버 내 폴더를 지정하고 웹브라우저의 요청에 따라 열람이 가능하게 하고있습니다. 홈페이지는 메뉴 하나마다 html을 반환하는 node js로 작성된 서버로 유지하고 있습니다. (해당 서버는 전체적 프레임을 잡아주는 메인 html에서 메뉴 클릭시 이동할 수 있도록 설계하였습니다.)
